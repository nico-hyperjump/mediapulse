...
```typescript
{
  ...
  prompts?: object                    // Prompt templates snapshot (JSONB) - from AgentPromptVersion
...
**Note**: `AgentVersion` stores snapshots of configurations and prompts, but does not replace `AgentConfig`. When a version is deployed, the agent reads from `AgentConfig` (which may be updated to match the version's config). The version serves as a historical record and rollback point.
...
- Deployment updates `AgentVersionDeployment` table to mark the active version
- The agent's `AgentConfig` is updated to match the version's configuration (or agents read version config directly)
- Previous production version automatically tracked for rollback
- Agent instances reload configuration from database (hot-reload without restart, but may require agent re-initialization)
...
- Agent's `AgentConfig` is updated to match the previous version's configuration
