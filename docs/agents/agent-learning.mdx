...
Collect metrics, analyze user feedback, optimize content generation, and manage A/B tests. The Learning Agent runs independently on a schedule (typically daily at midnight) and creates optimized agent versions for admin review and deployment.
...
**Schedule**: Typically runs daily at midnight (configurable via Scheduler Agent or cron). Can also be triggered manually via admin interface.
