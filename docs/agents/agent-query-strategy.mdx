...
2. **Dynamic Query Generation**:
   ...
   - Creates queries for news APIs, social media, and web scraping
...
5. **Context-Aware Query Building**:
   ...
   - Adjusts queries based on market conditions
   - Incorporates seasonal/cyclical patterns
   - Adapts to breaking news or market volatility
  ...
  entityDiscovery: {
    ...
    entityTypes: {
      ...
      industryPeers: {
        enabled: boolean,
        sameSector: boolean,
        sameMarketCap: boolean,
        sameRegion?: boolean           // Optional: limit to same market region
      }
    },
    refreshInterval: {
...
```typescript
{
  ...
  entityGraph: {
    ...
    entities: {
      ...
      industryPeers: Array<{
        ...
        companyName: string,
        similarityFactors: string[],    // ['sector', 'marketCap', 'revenue']
        lastUpdated: Date
      }>
    },
    relationships: Array<{
  ...
  queries: {
    news: Array<{
      id: string,
      query: string,
      source: string,                    // 'newsapi', 'google', 'bloomberg', etc.
...
- Load previous query performance data
...
- Save the entity graph to the database
- Store the generated queries
- Update the query performance metrics
...
<Mermaid
  chart="
flowchart TD
    A[Scheduler] --> B[Query Strategy Agent]
    ...
    I -->|No| K[Return: Entity Graph + Queries + Keywords]
    J --> K
    K --> L[Data Collection Agent uses queries]
"
/>

## Integration with Data Collection Agent

- Query Strategy Agent runs **before** Data Collection Agent
- Data Collection Agent receives generated queries and uses them for targeted searches
- Data Collection Agent provides feedback (results, relevance scores) back to Query Strategy Agent
- Query Strategy Agent uses feedback to optimize future queries
