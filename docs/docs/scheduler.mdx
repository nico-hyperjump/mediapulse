...
**Note**: The orchestrator interacts with the [Agent Registry API](./agent-registry-api) to discover available agent instances, monitor their health, and track capacity. The Agent Registry API is a separate service that agents use to register themselves and report their status. See [Agent Registry API Documentation](./agent-registry-api) for details.
...
### `AgentRegistry`
Stores **agent type metadata** (not version-specific). Agents register themselves via the [Agent Registry API](./agent-registry-api). See [Agent Registry API Documentation](./agent-registry-api#agentregistry) for the complete schema definition and [Agent Specification](./agent-specification) for full registration details.

### `AgentInstance`
Stores running agent instances managed by the orchestrator. The orchestrator automatically creates, manages, and terminates instances. Multiple instances of the same agent version can exist for horizontal scaling. Instances register themselves and report status via the [Agent Registry API](./agent-registry-api). See [Agent Registry API Documentation](./agent-registry-api#agentinstance) for the complete schema definition and [Agent Specification](./agent-specification) for instance lifecycle details.
...
2. **Agent Discovery & Instance Management**:
   ...
   - **Interact with Agent Registry API**:
     - Query Agent Registry API to discover available agent instances (or query `AgentInstance` table directly)
     - Monitor instance health and availability through Agent Registry API
   ...
   - Query `AgentInstance` table for all active instances of each agent version (via Agent Registry API or direct database query)
