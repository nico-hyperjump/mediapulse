...
- **Registered** in the `AnalysisTypeRegistry` database table
- **Loaded dynamically** by the Analysis Agent at runtime
- **Configured** via the `AgentConfig` table
- **Executed in parallel** with other analysis types
- **Automatically integrated** into content generation
