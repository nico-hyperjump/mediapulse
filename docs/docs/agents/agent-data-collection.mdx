...
Collect raw media data from multiple sources for specified companies within a time window. Handles web scraping, API calls, and data aggregation with intelligent deduplication and relevance scoring.
...
1. **Intelligent News Collection** (not blind scraping):
   ...
   - Searches for: company name, competitors, industry keywords, executives, related events
   - Filters by relevance before storing (not storing everything)

2. **Comprehensive Social Media Monitoring**:

   - Monitors company mentions directly
...
3. **Entity Relationship Usage**:

   - Uses entity graph from Query Strategy Agent: competitors, suppliers, customers, executives, industry peers
   - Expands search queries intelligently based on entity relationships
   - Note: Entity graph is maintained by Query Strategy Agent, not Data Collection Agent

4. **Earnings & SEC Filings** (if applicable):
...
interface DataCollectionInput {
  ticker: string; // Company identifier (e.g., "AAPL", "TSLA")
  timeWindow: {
    // Collection period
...
**3c. Earnings Calls** (if applicable):
...
flowchart TD
    ...
    B3 --> C[Parallel Collection]
    
    subgraph Collection [Parallel Collection - All Sources Concurrent]
        direction TB
        C1[News Scraper<br/>Bloomberg<br/>Reuters<br/>WSJ<br/>CNBC] --> C2[Social Monitor<br/>Twitter API<br/>Reddit API]
        C2 --> C3[Earnings Fetch<br/>SEC EDGAR<br/>Transcripts]
    end
