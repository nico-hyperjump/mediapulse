...
**Note**: `AgentVersion` stores snapshots of configurations and prompts, but does not replace `AgentConfig`. When a version is deployed:
...
- **Deployment Process**:
  ...
  2. `AgentConfig` table is updated to match the version's configuration from `AgentVersion.config`
