...
- ✅ Basic scheduler that can:
  ...
  - Create jobs for user-ticker combinations (one job per user-ticker pair)
  ...
  - Check data freshness before processing (basic check: compares `Newsletter.createdAt` timestamp to current time; if newsletter exists and is less than 24 hours old, skip processing)
