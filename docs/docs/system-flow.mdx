...
flowchart TB
    ...
    subgraph Newsletter [Newsletter Generation Pipeline]
        Scheduler[Scheduler Agent<br/>• Database-driven schedules<br/>• Admin-configurable<br/>• Executes pipelines from DB]
        Analysis[Analysis Agent<br/>• Triggered by Scheduler<br/>• Reads collected data from DB<br/>• Works independently]
        Content[Content Generation Agent<br/>• Triggered by Scheduler<br/>• Reads analysis from DB<br/>• Sequential per user<br/>• Works independently]
        QA[Quality Assurance Agent<br/>• Triggered by Scheduler<br/>• Reads newsletter from DB<br/>• Works independently]
        Delivery[Delivery Agent<br/>• Triggered by Scheduler<br/>• Reads approved newsletter from DB<br/>• Works independently]
    end
...
flowchart TD
    ...
    G --> H[Content Generation Agent<br/>• Triggered by Scheduler<br/>• Reads analysis from DB<br/>• Reads user prefs from DB<br/>• Generates newsletter<br/>• Writes newsletter to DB]
    H --> I[Quality Assurance Agent<br/>• Triggered by Scheduler<br/>• Reads newsletter from DB<br/>• Validates content<br/>• Checks compliance<br/>• Writes approval status to DB]
    I --> J{Approved?}
    J -->|Yes| K[Delivery Agent<br/>• Triggered by Scheduler<br/>• Reads approved newsletter from DB<br/>• Sends email<br/>• Updates dashboard<br/>• Writes metrics to DB]
    J -->|No| H
...
sequenceDiagram
    ...
    Note over Scheduler: Scheduled: Newsletter Generation Pipeline
    ...
    Scheduler->>Analysis: Trigger analysis (independent execution)
    Analysis->>DB: Read collected data
    Analysis->>Analysis: Perform analysis
    Analysis->>DB: Store analysis results

    Scheduler->>Content: Trigger content generation (independent execution)
    ...
    Scheduler->>QA: Trigger QA (independent execution)
    QA->>DB: Read newsletter draft
    QA->>QA: Validate content
    QA->>DB: Store approval status
    alt Approved
        Scheduler->>Delivery: Trigger delivery (independent execution)
        ...
        Note over Scheduler: Scheduler or retry mechanism<br/>can enqueue Content Gen retry job<br/>based on QA results
    end
