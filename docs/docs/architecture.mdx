...
**Note**: Agents are **language-agnostic** and can be implemented anywhere. They register themselves in the database and expose HTTP endpoints. See [Agent Specification](./agents/agent-specification) for details. The `packages/agents/` directory (if it exists) would contain shared utilities, SDKs, or helper libraries for agent development.
...
- `AgentRegistry` - Agent metadata and registration (agents register themselves here with HTTP endpoints)
